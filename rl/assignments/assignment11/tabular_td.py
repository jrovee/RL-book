...
def tabular_td_prediction(
    ...
    for step in transitions:
        ...
        n_seen[step.state] += 1
        values[step.state] += alpha * (step.reward + γ*values[step.next_state] - values[step.state])
...
if __name__ == '__main__':
    ...
    transitions = [step for trace in traces for step in trace]

    predictor = tabular_td_prediction(traces)
